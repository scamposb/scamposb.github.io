document.addEventListener("DOMContentLoaded", () => {
    ...
    function updatePollStatus(year) {
        fetch(`http://192.168.1.46:8080/poll?year=${year}`)
          .then((res) => res.json())
          .then((data) => {
            isOpenStatus.textContent = data.isOpen ? "✅ Abierta" : "❌ Cerrada";
            isOpenStatus.className = data.isOpen
              ? "text-green-400 font-bold"
              : "text-red-400 font-bold";
            toggleButton.textContent = data.isOpen ? "Cerrar votación" : "Abrir votación";
            toggleButton.dataset.open = data.isOpen;
          });
      }

    yearSelect.addEventListener("change", () => {
      const year = yearSelect.value;
      if (year) fetchResults(year);
    });
